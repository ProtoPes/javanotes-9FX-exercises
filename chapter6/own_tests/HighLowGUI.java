...
public class HighLowGUI extends Application {
    ...
    public void start(Stage stage) {
        ...
        higher.setOnAction( e -> isHigher(true) );
        Button lower = new Button("Lower");
        lower.setOnAction( e -> isHigher(false) );
...
//        HBox.setHgrow(newGame, Priority.ALWAYS); // not the same size.
    ...
    /**
     * Called by an event handler when user clicks "Higher" or
     * "Lower" button.
     * Check the user's prediction.  Game ends if user guessed
     * wrong or if the user has made three correct predictions.
     */
    private void isHigher(boolean prediction) {
        ...
        if ( prediction && thisCard.getValue() < prevCard.getValue() ) {
            gameInProgress = false;
            message = "Too bad! You lose.";
        }
        else if ( !prediction && thisCard.getValue() > prevCard.getValue() ) {
            gameInProgress = false;
            message = "Too bad! You lose.";
        }
